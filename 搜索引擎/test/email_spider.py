...
imageExt = ["jpeg", "exif", "tiff", "gif", "bmp", "png", "ppm", "pgm", "pbm", "pnm", "webp", "hdr", "heif", "bat",
            "bpg", "cgm", "svg", "jpg", "css", ".js", ".io", "ebp",'.xx']
ua = UserAgent()
...
class Spider:
    ...
    def generate_keywords_queue(self):
        ...
        line_count = 1
        logger("生成url队列中......请稍候")
        with open("./url_data/web.txt") as f:
            url = f.readline().strip()
            while url:
                url = f.readline().strip()
                self.url_queue.put(url)
                line_count+=1
        logger("url总数：{}".format(line_count))
    ...
    def extractOnlyUrl(self, url_queue,thread_number):
        global email_count
        global key_count
        key_index= key_count
        try:
            while not url_queue.empty():
                ...
                logger("线程：{}：当前爬取第{}个url：{}".format(thread_number,key_index, url))
                ...
                if len(listUrl) > 0:
                    for email in listUrl:
                        try:
                            ...
                            logger("线程：{}：正采集第{}个url：{}   Email为:【{}】:{}".format(thread_number,key_index, url,email_count, email), color="")
            ...
            self.extractOnlyUrl(url_queue,thread_number)

    def main(self):
        global start_keyword_index, res_set
        logger("当前已爬url数量：{}".format(start_keyword_index))
        ...
        for i in range(thread_num):
            thread = threading.Thread(target=self.extractOnlyUrl, args=(self.url_queue,i))
